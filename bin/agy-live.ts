...
interface AgySession { id: string; path: string; projectDir: string; model: string; size: number; mtime: number }
  ...
  try {
    ...
    while ((m = rx.exec(text)) !== null) {
      ...
      try {
        ...
        for (let i = 0; i < 6; i++) {
          if ([".git","settings.gradle","package.json"].some(f => fs.existsSync(path.join(dir, f)))) return dir
...
function findProjectSession(ss: AgySession[]): AgySession | null {
  ...
  return ss.find(s => s.projectDir !== "(Unbound session)" && cwd.startsWith(s.projectDir)) ?? null
}
...
function fmtSize(b: number): string { const k=b/1024; return k>1024?(k/1024).toFixed(1)+" MB":k.toFixed(0)+" KB" }
...
async function main() {
  ...
  if (!sessions.length) { console.error("❌ No agy sessions found in ~/.gemini/antigravity-cli/brain/"); process.exit(1) }

  const arg = process.argv[2]
  let currentSession = (arg ? sessions.find(s => s.id.startsWith(arg)) : null)
    ?? findProjectSession(sessions) ?? sessions[0]

  const renderer = await createCliRenderer({
    exitOnCtrlC: false, targetFps: 20,
...
class FastScrollAccel {
  ...
  const leftPane = new BoxRenderable(renderer, {
    flexDirection: "column", flexGrow: 1, height: "100%", overflow: "hidden",
  })

  const scrollBox = new ScrollBoxRenderable(renderer, {
    flexGrow: 1, width: "100%", stickyScroll: true, stickyStart: "bottom",
  ...
  const selBox = new ScrollBoxRenderable(renderer, {
    flexGrow: 1, width: "100%", backgroundColor: "#0d1117",
  })
  ...
  const footerBar = new BoxRenderable(renderer, {
    width: "100%", height: 1, backgroundColor: "#1a1a2e",
    flexDirection: "row", alignItems: "center", paddingX: 1, gap: 1,
  })
  const statusTxt = new TextRenderable(renderer, { content: "⠋ Loading...", fg: "#63b3ed", flexGrow: 1 })
  const liveTxt = new TextRenderable(renderer, { content: "[LIVE]", fg: "#68d391" })
  footerBar.add(statusTxt); footerBar.add(liveTxt)

  leftPane.add(scrollBox); leftPane.add(footerBar)
  ...
  const sidebar = new BoxRenderable(renderer, {
    ...
    flexDirection: "column", backgroundColor: "#111827",
    paddingX: 1, overflow: "hidden",
  })

  const sbHdr = new BoxRenderable(renderer, { width: "100%", height: 1, backgroundColor: "#0e7490" })
  sbHdr.add(new TextRenderable(renderer, { content: " ℹ️  MONITOR & STATS", fg: "#000000", attributes: BOLD_ATTR }))
  sidebar.add(sbHdr)
  ...
  function addSbRow(label: string, valFg="#e2e8f0"): TextRenderable {
    ...
    row.add(val); sidebar.add(row); return val
  }
  function addSbKey(key: string, desc: string) {
    const row = new BoxRenderable(renderer, { width: "100%", height: 1, flexDirection: "row", gap: 1 })
  ...
  vProjBox.add(vProjVal1); vProjBox.add(vProjVal2)
  ...
  vSessBox.add(vSessVal1); vSessBox.add(vSessVal2)
  ...
  vModelBox.add(vModelVal1); vModelBox.add(vModelVal2)
  ...
  const vCtxBarTxt = new TextRenderable(renderer, { content: "  [░░░░░░░░░░░░] 0%", fg: "#94a3b8" })
  vCtxBar.add(vCtxBarTxt); sidebar.add(vCtxBar)

  const vQuota5h = addSbRow("• 5h Quota:", "#4ade80")
  const vQuota5hBar = new BoxRenderable(renderer, { width: "100%", height: 1 })
  const vQuota5hBarTxt = new TextRenderable(renderer, { content: "  [░░░░░░░░░░░░] --%", fg: "#94a3b8" })
  vQuota5hBar.add(vQuota5hBarTxt); sidebar.add(vQuota5hBar)

  const vQuotaWk = addSbRow("• Wk Quota:", "#fbbf24")
  const vQuotaWkBar = new BoxRenderable(renderer, { width: "100%", height: 1 })
  const vQuotaWkBarTxt = new TextRenderable(renderer, { content: "  [░░░░░░░░░░░░] --%", fg: "#94a3b8" })
  vQuotaWkBar.add(vQuotaWkBarTxt); sidebar.add(vQuotaWkBar)
  ...
  renderer.root.add(leftPane); renderer.root.add(sep); renderer.root.add(sidebar)

  // ── State ─────────────────────────────────────────────────────────────────────
  let currentPos = 0, remainder = "", stepCount = 0, pushCount = 0
  let lastActivityMs = Date.now()
  let activeFileFd: number | null = null
  let spinnerIdx = 0, spinnerTimer: ReturnType<typeof setInterval> | null = null
  let totalPromptChars = 0, totalThinkingChars = 0, totalOutputChars = 0
  let isLive = true
  let scheduledUntilMs = 0
  let scheduledPrompt = ""
  let activeBackgroundTask: string | null = null
  let currentModel = currentSession.model || "Detecting..."
  let userPromptCount = 0, plannerResponseCount = 0, toolCallCount = 0, checkpointCount = 0
  ...
  function makeMeterBar(pct: number, width = 12, remainingMode = false): { bar: string; color: string } {
  ...
  function fetchLiveQuotaAsync(onDone?: () => void) {
    ...
    proc.on("error", () => { isFetchingQuota = false })
  }

  function getModelContextLimit(modelName: string): number {
    const norm = (modelName || "").toLowerCase()
    if (norm.includes("gemini") || norm.includes("flash") || (norm.includes("pro") && !norm.includes("claude"))) {
      return 1_000_000
    }
    if (norm.includes("claude") || norm.includes("sonnet") || norm.includes("opus") || norm.includes("haiku")) {
      return 200_000
    }
  ...
  function updateSidebar() {
    ...
    const folder = currentSession.projectDir !== "(Unbound session)"
      ? path.basename(currentSession.projectDir) : "Unbound"
    ...
    const isClaude = model.toLowerCase().includes("claude") || model.toLowerCase().includes("sonnet") || model.toLowerCase().includes("gpt")
    ...
    const inRate = isClaude ? 3.00 : 0.15
    const outRate = isClaude ? 15.00 : 0.60
  ...
  function startSpinner(txt: string) {
    ...
    spinnerTimer = setInterval(() => {
      ...
      if (now < scheduledUntilMs) {
        ...
        setStatus(`${SPINNER[spinnerIdx++ % SPINNER.length]} ⏳ [SCHEDULE] "${scheduledPrompt || "Waiting"}" (${rem}s)`)
      } else {
  ...
  function parseMarkdownSpans(line: string, defaultFg = "#ffffff"): { text: string; bold?: boolean; fg?: string }[] {
  ...
  function renderStep(step: any) {
    ...
    if (step.content && ["USER_INPUT", "GENERIC", "SYSTEM_MESSAGE", "CHECKPOINT"].includes(step.type)) {
      totalPromptChars += String(step.content).length
      if (typeof step.content === "string" && step.content.includes("Model Selection")) {
        const match = step.content.match(/setting `Model Selection`.*?\bto\s+([^<\n]+?)(?:\.\s|\.\n|\.<|$)/i)
        if (match) {
    ...
    try { currentSession.size = fs.fstatSync(activeFileFd!).size } catch {}
    ...
    if (step.type === "SYSTEM_MESSAGE" && step.content) {
      ...
      } else if (raw.includes("exited with code") || raw.includes("error") || raw.includes("Error")) {
    ...
    if (step.type === "PLANNER_RESPONSE") {
      ...
      if (step.content) {
        ...
        if (clean) {
          pushLine("")
          pushLine(hasTools ? "  💬 [Assistant]" : "  💬 [Assistant Response]", hasTools ? "#38bdf8" : "#4ade80")
            ...
            case "write_to_file": {
              ...
              if (raw) {
                for (let k = 0; k < lines.length; k++) {
                  cardLines.push({ text: `  + ${String(1 + k).padStart(4)}: ${lines[k]}`, fg: "#4ade80" })
                }
              }
              pushCard(cardLines)
              break
            ...
            case "replace_file_content": {
              ...
              for (let k = 0; k < total; k++) {
                if (k < tLines.length && k < rLines.length && tLines[k] !== rLines[k]) {
                  cardLines.push({ text: `  - ${String(sLine + k).padStart(4)}: ${tLines[k]}`, fg: "#f87171" })
                  cardLines.push({ text: `  + ${String(sLine + k).padStart(4)}: ${rLines[k]}`, fg: "#4ade80" })
                } else if (k < tLines.length && k >= rLines.length) {
                  cardLines.push({ text: `  - ${String(sLine + k).padStart(4)}: ${tLines[k]}`, fg: "#f87171" })
                } else if (k < rLines.length && k >= tLines.length) {
                  cardLines.push({ text: `  + ${String(sLine + k).padStart(4)}: ${rLines[k]}`, fg: "#4ade80" })
                } else if (k < rLines.length) {
                  cardLines.push({ text: `    ${String(sLine + k).padStart(4)}: ${rLines[k]}`, fg: "#94a3b8" })
                }
              }
              pushCard(cardLines)
              break
  ...
  function switchSession(s: AgySession) {
    if (activeFileFd!=null) { try{fs.closeSync(activeFileFd)}catch{}; activeFileFd=null }
    currentSession=s; currentPos=0; remainder=""; isLive=true
    currentModel = s.model || "Detecting..."
    clearScrollBox(); resetCounters()
    updateSidebar()
  ...
  let isSelectingSession = false, selectorCursor = 0
  ...
  function renderSelectorList() {
    ...
    const maxItems = Math.max(1, Math.min(cachedSessions.length, Math.floor(((renderer.height || 24) - 4) / 2)))
    for (let i = 0; i < maxItems; i++) {
      const s = cachedSessions[i], sel = i === selectorCursor
      ...
      topRow.add(new TextRenderable(renderer, { content: sel ? "▶" : " ", fg: "#fbbf24", width: 2 }))
      topRow.add(new TextRenderable(renderer, { content: String(i + 1).padStart(2), fg: "#6b7280", width: 2 }))
      topRow.add(new TextRenderable(renderer, {
        content: `📁 ${folder}`, fg: sel ? "#ffffff" : "#38bdf8",
        attributes: sel ? BOLD_ATTR : 0, flexGrow: 1,
      }))
      topRow.add(new TextRenderable(renderer, { content: fmtSize(s.size), fg: "#6b7280", width: 8 }))
      topRow.add(new TextRenderable(renderer, { content: timeAgo(s.mtime), fg: sel ? "#fbbf24" : "#6b7280", width: 10 }))
      itemBox.add(topRow)

      const botRow = new BoxRenderable(renderer, {
        width: "100%", height: 1, flexDirection: "row", paddingLeft: 5, gap: 2,
      })
      botRow.add(new TextRenderable(renderer, {
        content: `🆔 ${s.id}`, fg: sel ? "#93c5fd" : "#64748b",
      }))
      botRow.add(new TextRenderable(renderer, {
        content: `🤖 ${s.model}`, fg: sel ? "#fde047" : "#a78bfa",
      }))
      itemBox.add(botRow)
  ...
  function renderContextView() {
    ...
    const estimatedActiveTokens = Math.round((activeContextChars + 32000) / 4)
    const pct = Math.min(100, (estimatedActiveTokens / limit) * 100)
    const folder = currentSession.projectDir !== "(Unbound session)" ? currentSession.projectDir : "(Unbound session)"

    function addContextCard(title: string, lines: { label: string; value: string; valFg?: string }[]) {
      const card = new BoxRenderable(renderer, {
        width: "100%", flexDirection: "column", backgroundColor: "#1e293b",
        paddingX: 1, marginY: 1,
      })
      const hdr = new BoxRenderable(renderer, { width: "100%", height: 1 })
      hdr.add(new TextRenderable(renderer, { content: `▎ ${title}`, fg: "#38bdf8", attributes: BOLD_ATTR }))
      card.add(hdr)
      for (const row of lines) {
        const r = new BoxRenderable(renderer, { width: "100%", height: 1, flexDirection: "row" })
        r.add(new TextRenderable(renderer, { content: `  ${row.label}`, fg: "#94a3b8", width: 24 }))
        r.add(new TextRenderable(renderer, { content: row.value, fg: row.valFg || "#ffffff", flexGrow: 1 }))
        card.add(r)
      }
      contextBox.add(card)
    }
    ...
    addContextCard("SESSION & MODEL OVERVIEW", [
      ...
      { label: "Context Window Limit:", value: `${(limit / 1000).toLocaleString()}K tokens (100%)`, valFg: "#e2e8f0" },
    ...
    addContextCard("LIVE CONTEXT LOAD IN MEMORY", [
      { label: "Active Window Load:", value: `${(estimatedActiveTokens / 1000).toFixed(1)}K / ${(limit / 1000).toFixed(0)}K tokens`, valFg: "#38bdf8" },
      { label: "Context Utilization:", value: `${pct.toFixed(2)}% used`, valFg: barColor },
      { label: "Visual Meter:", value: `[${barStr}]`, valFg: barColor },
      { label: "Compacted Slices:", value: `${checkpointCount} checkpoints (auto-compacted)`, valFg: "#a78bfa" },
    ])
    ...
    const tt = Math.round(totalThinkingChars / 4)
    const ot = Math.round(totalOutputChars / 4)
    const isClaude = model.toLowerCase().includes("claude") || model.toLowerCase().includes("sonnet")
    const inRate = isClaude ? 3.00 : 0.15
    const outRate = isClaude ? 15.00 : 0.60
    const estDollar = ((pt / 1_000_000) * inRate) + (((tt + ot) / 1_000_000) * outRate)

    addContextCard("CUMULATIVE SESSION TOKENS (~chars / 4)", [
      { label: "Prompt Input:", value: `${fmtTokens(pt)} tokens (${totalPromptChars.toLocaleString()} chars)`, valFg: "#e2e8f0" },
      { label: "Thinking / CoT:", value: `${fmtTokens(tt)} tokens (${totalThinkingChars.toLocaleString()} chars)`, valFg: "#c084fc" },
      { label: "Tool Output & Gen:", value: `${fmtTokens(ot)} tokens (${totalOutputChars.toLocaleString()} chars)`, valFg: "#4ade80" },
    ...
    quotaRows.push(
      { label: "Est. API Reference Cost:", value: `$${estDollar.toFixed(2)} (Simple estimate)`, valFg: "#38bdf8" },
      { label: "Quota Limit Status:", value: "✅ Normal (Active subscription / No 429 throttle)", valFg: "#22c55e" }
    )
    ...
    addContextCard("INJECTED DIRECTIVES & RULES", [
      { label: "Global Protocol:", value: "Caveman Communication + Ponytail Engineering Ladder", valFg: "#4ade80" },
      { label: "Project Architecture:", value: "Clean Architecture MVVM (Pure Kotlin Domain)", valFg: "#38bdf8" },
      { label: "CodeGraph / Memory:", value: "Auto-Recall agentmemory + CodeGraph MCP", valFg: "#fbbf24" },
    ])
  }
  ...
  function exitApp(code = 0) {
    ...
    if (spinnerTimer) clearInterval(spinnerTimer)
    if (activeFileFd != null) { try { fs.closeSync(activeFileFd) } catch {} }
  ...
  function handleKey(name: string, seq = "", ctrl = false, shift = false) {
    ...
    if (keyLower === "pageup") {
      isLive = false
      scrollBox.stickyScroll = false
      scrollBox.scrollTop = Math.max(0, scrollBox.scrollTop - Math.max(1, (renderer.height || 24) - 4))
      updateLiveLabel()
      renderer.requestRender()
      return
    }
    if (keyLower === "pagedown") {
      scrollBox.scrollTop = Math.min(scrollBox.scrollHeight, scrollBox.scrollTop + Math.max(1, (renderer.height || 24) - 4))
      if (scrollBox.scrollTop >= scrollBox.scrollHeight - 2) {
        isLive = true
        scrollBox.stickyScroll = true
      }
  ...
  process.on("uncaughtException", (err) => {
    try { fs.writeFileSync("/tmp/agy-crash.log", (err?.stack || String(err)) + "\n") } catch {}
    exitApp(1)
  })
